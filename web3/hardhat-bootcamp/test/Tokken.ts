...
describe('Ardarod', function () {
    async function deployToken() {
        const signers = await ethers.getSigners();
        const Token = await ethers.getContractFactory("Ardarod");
    ...
    it('Should have the correct name, symbol, and decimals', async function () {
        ...
        expect(await token.name()).to.eq("Ardarod");
        expect(await token.symbol()).to.eq("ARD");
    ...
    it('Should mint initial supply to the owner', async function () {
        ...
        expect(ownerBalance).to.eq(ethers.parseEther("1000000"));
    });
});